...
\begin{document}
	...
	Notice that another way to state the behavior of this machine is the following: suppose that $s$ is a string of $S^*$, $q$ is any string, and $t$ is a sequence that begins with an element not in $S$. Then, $\RT(S)$ takes the tape state $q |\rangle s t$ and returns the tape $q s |\rvert t$.
