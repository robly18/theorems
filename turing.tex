...
\begin{document}
	...
	\begin{align*}
	\P(k) =& \REMONE(inp, out)^k
	\end{align*}
	
	If the input is $\ceil{x_1} 0 \ceil{x_2} 0 \cdots 0 \ceil{x_n}$, after running $\P(k)$ with $k < n$, we will have $\ceil{x_k}$ on the output variable. This can easily be shown by induction on $k$.
