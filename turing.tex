...
\begin{document}
	...
	Suppose $g$ is a P-computable function $\N^n \rightharpoonup \N$, with a machine $G$ that takes a variable $inp_g$ and writes $g$ of it to $out_g$. Likewise, let $h$ be a P-computable function $\N^{2+n} \rightharpoonup \N$ that is computed by $H$, which writes $h(inp_h)$ to $out_h$.
	
	Now, to write a program that computes $\rho(g, h)$. First, suppose it takes the input in a variable $inp$. Begin by running %todo remone
