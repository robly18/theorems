...
\begin{document}
	...
	\end{proof} %pretty sure im complicating this proof.
	...
	\item (Primitive recursion) Suppose $h : \N^{n+2} \rightharpoonup \N$ is P-computable. Then, the function $f$ (denoted $\rho(h)$) defined inductively as:
	\begin{gather*}
	f(0, x_1, x_2, \cdots, x_n) = (x_1, \cdots, x_n)\\
	...
	Now, we move on to composition. This one is not particularly hard.
	
	Let $f : \N^n \rightharpoonup \N^m$ and $g : \N^m \rightharpoonup \N^k$ be P-computable. Let $F$ be a P-machine that takes a variable $inp$ and writes $f(inp)$ to a variable $out_1$, and let $G$ be a P-machine that takes a variable $out_1$ and writes $g(out_1)$ to $out$. Composing the two machines $F;G$ will get us a machine that computes $g \circ f$.
	
	Next up is primitive recursion. This is possibly the hardest of the four.
	
	Let $h$ be a P-computable function $\N^{2+n} \rightharpoonup \N$ that is computed by $H$, which writes $h(inp_h)$ to $out_h$.
	
	Now, to write a program that computes $\rho(h)$. First, suppose it takes the input in a variable $inp$. Begin by running $\REMONE(inp, ix)$. This wil take the $n+1$-uple $(y, x_1, \cdots, x_n)$ contained in $inp$, and write $y$ to $ix$, leaving $(x_1, \cdots, x_n)$ in $inp$.
	
	Now, if $y = 0$, we are done, so we just $\COPY(inp, out)$. Otherwise, however, we want to run $h$ and decrease $y$ by one, and repeat this until $y$ is in fact zero. Consider, then, the following program: (where $ix2$ is a variable that starts empty)
	
	\begin{align*}
	&\COPY(inp, inp_g);\\
	&G;\\ %todo write out what g is
	&\COPY(out_g, out_h);\\
	&\WHILE(ix,\\
	&\quad \COPY(inp, inp_h);\\
	&\quad \PREPEND(0, inp_h); \COPY'(out_h, inp_h);\\
	&\quad \PREPEND(0, inp_h); \COPY'(ix2, inp_h);
	&\quad H\\
	&\quad \POP(ix); \PREPEND(1, ix2)\\
	&);\\
	&\COPY(out_h, out)
	\end{align*}
	
	Let us remind ourselves that, at the start of the while, we have the current state:
	
	$ix$ contains $y$, $ix2$ contains 0, and $out_h$ contains $g(x_1, \cdots, x_n)$.
	
	If $y = 0$, the loop won't run and we will just write $g(x_1, \cdots, x_n)$ to the output, as desired. Otherwise, let us suppose the loop runs at least once.
	
	We will copy $inp$ to $inp_h$, leaving $inp_h$ with the value $(x_1, \cdots, x_n)$. The next two lines turn $inp_h$ into the $n+2$-uple $(ix2, out_h, x_1, \cdots, x_n)$, which, keeping in mind the assignment of variables, is the $n+2$-uple $(0, g(x_1, \cdots, x_n), x_1, \cdots, x_n)$.
	
	Then we run $h$ on this, leaving $out_h$ with the value $h(0, g(x_1, \cdots, x_n), x_1, \cdots, x_n)$, which is $\rho(h,g)(1, x_1, \cdots, x_n)$. Finally, we pop $ix$, leaving it at $y-1$, and prepend 1 to $ix2$, leaving it at $1$.
	
	If $y = 1$, we exit the loop with the correct output, otherwise we run it again, a total of $y$ times.
	
	Formalizing the situation: notice that, when we enter the loop, we are at situation
	...
	More generally, the situation is of the form
