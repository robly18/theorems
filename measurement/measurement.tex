...
Fixed a measurement $m$ and an open $U$, why should the set $(m \cdot)^{-1}(U)$ be open? That would be to say that it corresponds to a physical property that can be recorded with finite information. The physical property this corresponds to is: `if we have just measured $m$ then the system is compatible with $U$'.

For example, consider a system composed of two independent electrons, let us call them 1 and 2, whose spin is being measured in the $z$ axis. There are (at least) four possible measurements: $1^\uparrow, 1^\downarrow, 2^\uparrow$ and $2^\downarrow$, plus their compositions when they make sense. Since there is a finite set of possible combinations of states, every possible combination is finitely recordable, so we imbue the set of measurements with the discrete topology.

Now, let $U =  the set $(1^\downarrow \cdot)^{-1}(
\end{problem}
