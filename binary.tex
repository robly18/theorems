...
Hausdorff Measure is a central topic in Fractal Geometry. It is a generalization of the concept of length, area and volume for arbitrary Metric spaces and possibly nonintegral dimensions.

More concretely, fixed a metric space $X$ there exists a family of functions $\HH_s : P(X) \to \left[0, \infty \right]$, for $s \in \R^+_0$, representing the `$s$-dimensional Hausdorff measure'. So $\HH_1$ would correspond to `length', $\HH_2$ corresponds to `area', and so on.

Fixed a set $E \subseteq X$ there exists some $s \in \R^+_0$, called \emph{the dimension of $E$}, such that:

\begin{itemize}
\item For $t < s$ we have $\HH_t(E) = \infty$,

\item For $t > s$ we have $\HH_t(E) = 0$.
\end{itemize}

This concept has wide applications. For example, the Kakeya conjecture, which regards the Hausdorff dimension of a specific set, has far reaching applications in harmonic analysis.

An important tool for calculating dimensions of sets in $\R^n$ is the so-called \emph{binary measure}, denoted $\BB_s : P(X) \to \left[0, \infty \right]$. It is an estimate of the Hausdorff measure.

To calculate the Hausdorff measure of a set $E$, one must take the infimum of a certain sum over coverings of $E$ by arbitrary sets. In defining the binary measure one simply restricts such coverings to allow only the so-called binary intervals, which form a net, allowing some minimality arguments that would otherwise not be possible.

The reason these measures allow us to calculate dimensions of sets is the following inequality, which is true for any $s$ and $E \subseteq \R^n$:

\[ \HH_s(E) \leq \BB_s(E) \leq C_n \HH_s(E)\],

For some $C_n$.

The constant $C_n$ found in \cite{falconer} is $2^{n^2} 3^n$. For $n = 1$ this equates to 6. In this article, we seek to decrease this constant for this particular $n$, in order to find sharper bounds.

We also generalize the concept of binary measure, creating the so-called $b$-ary measures, for $b \in \N_{\geq 2}$. Likewise, these measures, denoted $\BB_{bs}$, also obey a similar inequality.

In the search for sharper bounds, we find a connection between the theory of $b$-ary measures and non-integer base digit expansions, and with it find an upper and a lower bound for the optimal constant as a function of the base $b$ and dimension $s$, as well as methods to compute these.

To conclude, we use these methods to investigate the relation between the $b$-ary measures for different $b$.

It should be expected that, in general, as $b$ increases the estimate of $\HH_s$ through $\BB_{bs}$ gets increasingly worse. This is, in fact, true for multiples. That is, if $b$ is a multiple of $c$, the estimate given by $\BB_{bs}$ is never better than that given by $\BB_{cs}$. However, the possible conjecture `as $b$ increases the estimate gets worse' is not always the case, as we show at the end of the article, by constructing a set $E$ and a dimension $s$ such that $\BB_{5s}(E) < \BB_{4s}(E)$.
