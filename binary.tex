...
Clearly, $\sigma_0 = 2 \gamma$. Furthermore, by the recursive definition of $S_n$, $\sigma_n$ is a geometric series, whose ratio between successive terms is simply $\frac{\sigma_1}{2 \gamma}$.

The only way for the limit to be nontrivial is for this series to be the constant one, and so for $\sigma_1$ to be $2 \gamma$. As such, let us calculate $\sigma_1$.
...
Finally, if we have a set of intervals with Hausdorff sum $H$ and apply this to all of them, the resulting Hausdorff sum is given by:

\begin{align*}
\sum (2 \gamma)^s \ell^s b^{\alpha - s - (\alpha + \beta) s} &=
(2 \gamma)^s b^{\alpha - s - (\alpha + \beta) s} \sum \ell^s\\
&= (2 \gamma)^s b^{\alpha - s - (\alpha + \beta) s} \cdot H
\end{align*}
