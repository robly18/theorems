...
We will now show the following: we can assume, in addition, that our interval contains the origin.

Let $b^j$ be the largest power of $b$ strictly less than $\ell$, so $b^j < \ell \leq b^{j+1}$, and let $I$ be an interval of length $\ell$. There exists at least one number of the form $k b^j$ contained in $I$. Let $I'$ be the interval $I$, translated such that this point becomes the origin. We will show that $\BB_{bs}^\infty(I') \geq \BB_{bs}^\infty(I)$, and so that, if we consider the interval $I'$ in our supremum, we do not need to look at $I$.

First, consider the following two cases: either $\BB_{bs}^\infty(I) = \BB_{bs}^{b^{j+1}}(I)$, or $\BB_{bs}^\infty(I) < \BB_{bs}^{b^{j+1}}(I)$.

In the first case, we need only consider coverings with intervals of length $b^j$ or below. Notice that these intervals are preserved by the translation that took $I$ to $I'$, and as such $\BB_{bs}^{b^j}(I) = \BB_{bs}^{b^j}(I')$. Hence:

\[\BB_{bs}^\infty(I) = \BB_{bs}^{b^j}(I) = \BB_{bs}^{b^j}(I') \leq \BB_{bs}^\infty(I')\]

As desired. We now proceed to inspect the second case: suppose $\BB_{bs}^\infty(I) < \BB_{bs}^{b^j}(I)$. In this case, there exists a $b$-ary covering of $I$ with sum less than any which uses only intervals of length $b^j$ or less. This covering must use at least one interval of size $b^{j+1}$ or above.

There are now two cases: either this interval covers $I$ entirely or it does not.

In this last case, suppose `retroactively' that the point we have chosen to translate $I$ by to become $I'$ was the end of the big interval that is contained in $I$. 
