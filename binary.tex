...
We leave the proof of the following elementary propositions to the reader:
...
Any of these covers $t$, and so we can easily show that:

\[ g(t) \leq \inf \{\, b^{ks} \times d_0 . d_1 d_2 \cdots d_{n-1} (d_n + 1) \mid n \in \N\,\}\]

It is not terribly surprising that this is, in fact, an equality.

\begin{prop}
Let $t$ have $b$-ary expansion $b^k \times d_0 . d_1 d_2 \dots$, with $d_0 = 0$. Then:

\[ g(t) = \inf \{\, b^{ks} \times d_0 . d_1 d_2 \cdots d_{n-1} (d_n + 1) \mid n \in \N\,\}\]
\end{prop}

\begin{proof}
We have already shown $LHS \leq RHS$, so we will focus on showing the other inequality.

We will do this in two parts: first, we will show that the infimum to calculate $g(t)$ can be taken over coverings of the form $b^k [a_0 a_1 a_2 \dots]$. Then, we will show that these $a_i$ can be taken to be only those of the above form.
...
For each interval $J \in A'$ collect the intervals $I \in A$ contained in $J$. We will show that each of these collections can be replaced by $J$ without raising the Hausdorff sum, which allows us to conclude, then, that the Hausdorff sum of $A'$ is $\leq$ that of $A$.
...
This concludes the first part of the proof. Fortunately, the second part is a lot more expedite.

We have already concluded we need only consider coverings of the form $b^k [a_0 a_1 a_2 \dots]. We will now show that we may restrict our attention to those sequences of the form $d_0 d_1 \cdots d_{n-1} (d_n + 1)$.

Fix any arbitrary sequence $\{a_n\}$ such that $b^k \times a_0 . a_1 a_2 \dots \geq b^k \times d_0 . d_1 d_2 \dots$. Either these two sequences are the same, or there exists a first index $j$ where they differ.
