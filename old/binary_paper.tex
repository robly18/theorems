...
\draw[thick] (8.6, 0.1) node[above]{t = 0.86} -- (8.6, -3.1);
 
\end{tikzpicture}
}
\caption{A visual representation of the procedure outlined to find upper bounds for $g(t)$}
\end{figure}
...
%The following was automatically generated with the help of the code at binary2.py
...
\draw[thick, gray, {[-)}] (0.00, -2) -- (3.33, -2);
\draw[thick, gray, {[-)}] (3.33, -2) -- (6.67, -2);
\draw (10,-2) node[right] {Contained};


\draw[thick, gray, {[-)}] (0.00, -3) -- (3.33, -3);
\draw[thick, gray, {[-)}] (3.33, -3) -- (6.67, -3);
\draw[thick, red, {[-)}] (6.67, -3) -- (10, -3);
\draw (10,-3) node[right] {Not contained, hence $a_0 = 2$};


\draw[thick, gray, {[-)}] (0.00, -4) -- (3.33, -4);
\draw[thick, gray, {[-)}] (3.33, -4) -- (6.67, -4);
\draw[thick, gray, {[-)}] (6.67, -4) -- (7.78, -4);
\draw (10,-4) node[right] {Contained};

\draw[thick, gray, {[-)}] (0.00, -5) -- (3.33, -5);
\draw[thick, gray, {[-)}] (3.33, -5) -- (6.67, -5);
\draw[thick, gray, {[-)}] (6.67, -5) -- (7.78, -5);
\draw[thick, red, {[-)}] (7.78, -5) -- (8.89, -5);
\draw (10,-5) node[right] {Not contained, hence $a_1 = 1$};


\draw[thick, gray, {[-)}] (0.00, -6) -- (3.33, -6);
\draw[thick, gray, {[-)}] (3.33, -6) -- (6.67, -6);
\draw[thick, gray, {[-)}] (6.67, -6) -- (7.78, -6);
\draw[thick, red, {[-)}] (7.78, -6) -- (8.15, -6);
\draw (10,-6) node[right] {Not contained, hence $a_2 = 0$};

\draw[thick, gray, {[-)}] (0.00, -7) -- (3.33, -7);
\draw[thick, gray, {[-)}] (3.33, -7) -- (6.67, -7);
\draw[thick, gray, {[-)}] (6.67, -7) -- (7.78, -7);
\draw (10,-7) node[right] {$A'$};
 
\end{tikzpicture}
}
\caption{A visual representation of the construction of $A'$. In blue is the original covering $A$. Note that the value of $t$ is irrelevant.}\label{constructAprime}
\end{figure}

First, we show that it covers the desired interval. We know that $A'$ covers the interval from 0 up to $a = b^k \times a_0 . a_1 a_2 \dots$, so suppose for the sake of argument that $a < t$. Recall that $t = b^k \times d_0 . d_1 d_2 \dots$, and hence it is easy to verify that if we had $a < t$ then there would exist $n$ such that $a_n < d_n$, but $a_i = d_i$ for $i < n$.\footnote{Implicit here is that these are nice (integer-based) digit expansions, and that all $d_i$ are less than $b$.} But if this were the case, we would have a contradiction with our definition of $a_n$ by maximality, as it would mean that $b^k \times a_0 . a_1 \cdots a_{n-1} (a_n + 1) \leq t$ and so $b^k [a_0 \cdots a_{n-1} (a_n + 1)] \subseteq \bigcup I_i$.
...
\begin{proof}
For $t = 0$ the proof is trivial, so we assume $t > 0$. For the other case, we begin by sketching a slightly wrong proof, before correcting our mistake.

Suppose $t$ has $b$-ary expansion $b^k \times d_0 . d_1 d_2 \dots$, and define $t_n = b^k \times d_0 . d_1 d_2 \cdots d_{n-1} (d_n + 1)$.

Then, it is almost true that $s_n = i(t_n)$ for all $n$. Therefore,
\[g(t) = \inf s_n \overset{\text{(wrong!)}}{=} \inf i(t_n) \geq \inf_{x \geq t} i(x).\]

For the other direction, it suffices to show that if $x \geq t$ then $i(x) \geq g(t)$. To this effect, note that if $x \geq t$ then the digit expansion of $x$ induces a $b$-ary cover of $\left[0,x\right[$, whose Hausdorff sum of dimension $s$ is precisely $i(x)$. Hence, $i(x)$ is the Hausdorff sum of a $b$-ary cover of $\left[0,x\right[$, and hence of a cover of $\left[0,t\right[$, so we conclude $i(x) \geq g(t)$ by definition of $g$.

We now correct the mistake we previously made. The error was in the assumption that $s_n = i(t_n)$: This is true \emph{if the greedy expansion of $t_n$ is $b^k \times d_0 . d_1 d_2 \cdots d_{n-1} (d_n + 1)$}, and this hypothesis fails if $d_n + 1 = b$.

In other words, we have the equality
\[s_n = i(t_n) \text{, given that $d_n \neq b-1$.}\]

Therefore, it suffices to show that the infimum $\inf s_n$ be taken over the collection of $n$ such that $d_n \neq b-1$, because then we can do the (correct!) sequence of manipulations
\[g(t) = \inf s_n = \inf_{d_n \neq b-1} s_n = \inf_{d_n \neq b-1} i(t_n) \geq \inf_{x \geq t} i(x).\]

So, suppose that $d_n = b-1$. It is trivial to verify that
\begin{equation}\label{shiftsone}
s_{n-1} = s_n - b \cdot b^{(k-n)s} + b^{(k-(n-1))s} \leq s_n.
\end{equation}

Applying inequality \eqref{shiftsone} enough times, one concludes that $s_n$ can be replaced in the infimum by $s_{n'}$ for some $n' < n$ which satisfies $d_{n'} \neq b-1$. Applying this process to all $n$ with $d_n = b-1$, one then concludes $\inf s_n = \inf_{d_n \neq b-1} s_n$.
\end{proof}
...
Let $b \in \Z_{\geq 2}$ and $t > 0$. Define $h(s) = g_{bs}(t)$. Then, $h$ is a continuous function from $\left]0, 1 \right]$ to $\R$. Furthermore, for $t \in \left[0, 1 \right]$, $h$ is decreasing.
\end{prop}
