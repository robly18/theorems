...
\end{align*}
	...
	donde $\Gamma^e \vdash p(t_1, \dots, t_n)$ sse $I \Vdash p(t_1, \dots, t_n)$, como desejado.
	...
	Para perceber isto, é preciso fazer clara a distinção entre um termo fechado e um termo do universo. Em particular, é preciso perceber que \emph{existem elementos do universo que não podem ser representados por termos fechados}. Daremos um exemplo em breve, mas a conclusão que usar o conjunto dos termos fechados como universo pode não ser suficiente.
	
	Considere-se um conjunto $\Gamma_\Z$ de fórmulas que representa o conjunto dos axiomas dos inteiros, numa assinatura cujos símbolos de função são apenas $\{0, 1, +\}$. Então, a partir das propriedades dos inteiros é possível provar que $\Gamma_\Z \vdash \exists_x(1+x = 0)$. No entanto, este elemento $x$ (que sabemos representar o inteiro $-1$) \emph{não pode ser representada por um termo fechado}.
	
	Vejamos outra forma na qual os termos fechados não são suficientes para representar os elementos do universo. Considere-se a fórmula $\varphi : 0 \leq x$, e defina-se a notação $\varphi_x[t]$, em que $t$ é um termo, para significar a fórmula $0 \leq t$. Portanto, por exemplo, $\varphi_x[1]$ é a fórmula (verdadeira) $0 \leq 1$ e $\varphi_x[1+1]$ é a fórmula (também verdadeira) $0 \leq 1+1$. Note-se que a nossa assinatura não nos permite escrever o número $-1$, por exemplo. De facto, \emph{para qualquer termo fechado $t$, $\Gamma_\Z \vdash \varphi_x[t]$}. No entanto, é óbvio que nos inteiros a fórmula $\varphi : 0 \leq x$ nem sempre é verdadeira.
	
	É este desfazamento entre os termos representáveis pela linguagem e os elementos que o universo tem de ter que faz a abordagem acima falhar, nomeadamente quando tentarmos fazer o passo de indução para passar de $\varphi$ para $\forall_{\bf x} \varphi$.
	\end{obstaculo}
	
	O que será preciso adicionar para fazer o passo de indução? Ou seja, considere-se uma fórmula $\forall_{\bf x} \varphi$ e tente-se justificar que $\Gamma \vdash \forall_{\bf x} \varphi$ sse $I \Vdash \forall_{\bf x} \varphi$. O primeiro passo é averiguar o significado de $I \Vdash \forall_{\bf x} \varphi$. Desenrolando as definições, é possível verificar que $I \Vdash \forall_{\bf x} \varphi$ sse, para todos os elementos $u$ do universo $I$, $\rho \Vdash \varphi$, onde $\rho$ é uma valoração tal que $\rho(\bf x) = u$. No caso particular da estrutura de interpretação que estamos a construir o universo são os termos fechados, pelo que $u$ será um termo fechado $t$. Consequentemente, é possível provar (ver prova da proposição \ref{prop:gammahatsemantics} abaixo) que $\rho^F(\varphi) = \lt$ sse $\Gamma \vdash \varphi_{\bf x}[u]$, onde esta notação significa `substituir a variável $\bf x$ pelo termo $u$'. Assim sendo, desejamos mostrar:
	\[
	\Gamma \vdash \forall_{\bf x} \varphi \text{ sse } \Gamma \vdash \varphi_{\bf x}[u] \text{, para todo o termo fechado $u$.}
	\]
