...
\begin{document}
	...
	%todo começar com um "exemplo da vida real"
...
Dados dois vetores $x,y \in \R^n$, dizemos $x \leq y$ se $x_i \leq y_i$ para todo $i$.
	...
	\subsubsection{Desigualdades para igualdades (Forma padrão)}\label{formapadrao}
...
Caso contrário:
	...
	Na prática, esta condição não é de particularidade utilidade. Tem, no entanto, uma interessante aplicação teórica.
	
	Abstratamente falando, é difícil categorizar o conjunto dos problemas não-degenerados no sentido lato. O conjunto dos problemas não-degenerados no sentido estrito, pelo outro lado, é muito mais fácil de manusear, visto que depende apenas de $A$. Por sua vez, isto permite-nos justificar em termos teóricos a afirmação `Problemas degenerados são raros': de facto, é possível mostrar que, escolhida uma matriz $A$ `ao calhas', esta é degenerada (no sentido estrito) com probabilidade 0, o que implica que ela é não-degenerada no sentido lato com probabilidade 1. A prova disto não é aqui feita, visto que não está no âmbito da cadeira (e o autor não a sabe).
	
	A maior utilidade que a vasta maioria dos leitores verão para este critério, no entanto, é a sua aplicação aos testes da Sernadas.
	
	\subsection{Problemas na forma canónica}
	
	Recorde-se que o primeiro passo do algoritmo do simplexo requer conhecer um vetor básico não-degenerado. Em geral, encontrar tal coisa é não-trivial. Existe, no entanto, um caso em que isto se mostra fácil.
	
	Suponha-se dado um pol na forma canónica, $P$:
	...
	Sabemos, devido aos resultados da secção \ref{formapadrao}, que podemos transformar $P$ num pol na forma padrão, digamos $Q$:
	...
	Este é um problema na forma padrão, que, se assumirmos não-degenerado, sabemos resolver usando o algoritmo do simplexo; \emph{assumindo} que conseguimos arranjar um vetor básico não-degenerado.
	
	Mas, ora, se adicionarmos a hipótese $b \geq 0$, é fácil adivinhar o vetor $(x,y)$, com $x = 0$ e $y = b$. O leitor pode facilmente verificar que isto é de facto admissível e básico, pelo que podemos agora aplicar o algoritmo do simplexo e o problema fica resolvido.
	...
	\bibitem{axler} Linear Algebra Done Right, Sheldon Axler
	
	\bibitem{spivak} Calculus on Manifolds, Michael Spivak
	
	\bibitem{io} Foundations and Applications of Linear Optimization, Cristina Sernadas et al.
	\end{thebibliography}
