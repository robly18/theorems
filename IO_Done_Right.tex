...
Dados dois vetores $x,y \in \R^n$, dizemos $x \leq y$ se $x_i \leq y_i$ para todo $i$.
	...
	Este tipo de tradução tem a propriedade que não muda o conjunto admissível e conjunto solução, mas isso não é estritamente necessário.
	
	\subsubsection{Adição de positividades (Forma canónica)}
	
	É, em muitos casos, útil estar a estudar um problema de otimização em que sabemos que todas as variáveis são não-negativas. Assim sendo, vamos agora ver que podemos sempre, sem perda de generalidade, assumir que o nosso pol é da forma
