...
Dados dois vetores $x,y \in \R^n$, dizemos $x \leq y$ se $x_i \leq y_i$ para todo $i$.
	...
	Queremos agora investigar a intuição que o conjunto solução contém sempre um vértice, se não-vazio.
	...
	\draw[->] (0, 0) -- (-5, -5) node[anchor=west] {y};
	...
	Resta caraterizar no que consistem os vértices, mas, pela imagem, parecem ser os pontos com o máximo de zeros possíveis.
	
	Primeiramente, vamos %todo
	
	Nesta secção vamos tentar aplicar conhecimento da Álgebra Linear aos problemas de otimização linear, com o objetivo de arranjar forma de descobrir solução.
	
	Isto significa, no entanto, que, visto que AL é muito melhor a lidar com igualdades do que desigualdades, vamos começar a trabalhar com pols na forma padrão.
	
	Mais especificamente, para prevenir casos patológicos, vamos trabalhar em pols na chamada \emph{forma padrão reduzida}, isto é, pols da forma
	...
	Em que a matriz $A$, de tamanho $m \times n$, tem caraterística $m$.
	
	Na prática, este requerimento é muito fácil de obter. Por exemplo, qualquer pol na forma padrão que venha de um pol na forma canónica está na forma padrão reduzida (verifique).
	
	No que se segue, vai-nos ser útil considerar a seguinte abreviatura: seja $N$ o conjunto $\{1,2,\cdots,n\}$.
	\begin{definition}
	Seja $B \subseteq N$. Denotamos por $A_B$ a matriz obtida de $A$ removendo as colunas cujos índices não estão em $B$. Da mesma forma, dado um vetor $x \in \R^n$ usamos $x_B$ para significar o vetor obtido de $x$ removendo os índices que não estão em $B$.
	\end{definition}
	
	\begin{definition}
	Dizemos que $B \subseteq N$ é uma \emph{base de índices de $A$} se $\# B = m$ e a matriz (quadrada) $A_B$ é não-singular.
	\end{definition}
	
	\begin{definition}
	Um vetor $x \in \R^n$ é dito \emph{básico} se existe uma base $B \subseteq N$ tal que para todo $i \in N \setminus B$ se tem $x_i \neq 0$.
	\end{definition}
