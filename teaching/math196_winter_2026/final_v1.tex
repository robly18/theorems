...
\noindent Formulas I think are useful:
\begin{itemize}
\item $\det(AB) = \det A \det B = \det(BA)$, $\det(A^{-1}) = 1/\det A$, $\det A^\tr = \det A$.
...
\item Dot product: $\vec u \cdot \vec v = u_1 v_1 + \dots + u_n v_n = \Norm{\vec u} \Norm{\vec v} \cos\theta = \vec{u}^\tr \vec v$.
\item Norm: $\Norm{\vec v}=\sqrt{\vec v \cdot \vec v} = \sqrt{v_1^2 + \dots + v_n^2}$.
\item $\mathrm{Volume}(AP) = \lvert \det A \rvert \mathrm{Volume}(P)$.
\item Rank-Nullity Theorem: $\dim \im A + \dim \ker A = \mathrm{width}(A)$.
\item Area of a circle: $\pi r^2$. Perimeter of a circle: $2 \pi r$.
\end{itemize}
\noindent Formulas requested by students:
{\small
\begin{itemize}
\item Projection on line spanned by unit vector $\vec u$: $\vec u \vec u^\tr$, or $\mathrm{proj}_{\vec u}(\vec v) = (\vec v \cdot \vec u) \vec u$. $\mathrm{proj}_{\vec v_2}(\vec v_1) = \frac{\vec v_1 \cdot \vec v_2}{\vec v_2 \cdot \vec v_2} \vec v_2$.

\item $\vec a(t)=A^t \vec a(0)=SD^t S^{-1} \vec a(0)$.
\item Normal equation: $A^\tr A \vec x = A^\tr \vec b$.
\item $\ker A = \{\,\vec x \mid A\vec x = \vec 0\,\}$, $\im A = \{\,A\vec x \mid \vec x \in \R^n\,\}$.
\item Cauchy-Schwarz Inequality: $\abs{\vec x \cdot \vec y} \leq \Norm{\vec x} \Norm{\vec y}$.
\item Vectors $\vec v_1 ,\dots , \vec v_k$ are said to be \emph{linearly dependent} if there is a list of numebrs $c_1, \dots, c_k$, not all of them zero, such that $c_1 \vec v_1 + \dots + c_k \vec v_k = \vec 0$.
\item $\hat v = \frac{\vec v}{\Norm v}$.
\item If $\vec v_1, \dots, \vec v_k$ are an orthonormal basis for $V$, then $\mathrm{proj}_V(\vec x) = (x\cdot \vec v_1) \vec v_1 + \dots + (x \cdot \vec v_k) \vec v_k$.
\item $D = \left[\begin{smallmatrix} \lambda_1 & 0 & 0 \\
0 & \lambda_2 & 0 \\
0 & 0 & \lambda_3\end{smallmatrix}\right]$.
\end{itemize}
}
%\hrule
...
\[
\vec v_1 = \begin{bmatrix} 1 \\ 0 \\ 1 \\ 1 \end{bmatrix}
\quad
\vec v_2 = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 1 \end{bmatrix}
\quad
\vec v_3 = \begin{bmatrix} 2 \\ 1 \\ 2 \\ 4 \end{bmatrix}
\quad
\vec v_4 = \begin{bmatrix} 1 \\ 0 \\ 1 \\ 5 \end{bmatrix}
\]
\begin{parts}
\part[3] Determine if these vectors form a basis of $\R^4$.
\part[2] Determine if these vectors are linearly dependent or linearly independent.
\part[2] Determine if $\vec v_3$ is in $\mathrm{span}(\vec v_1, \vec v_2, \vec v_4)$.
\part[3] Determine if $\vec v_1$ is in $\mathrm{span}(\vec v_2, \vec v_3, \vec v_4)$.
\end{parts}
