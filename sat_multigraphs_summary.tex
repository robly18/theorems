...
On the other hand, let us suppose that $\rho$ does not satisfy $\Gamma$. We wish to show $\chi$ is not valid.
\end{proof}
...
\begin{proof}
First, let us suppose $\Gamma$ is satisfiable. Consider a satisfying valuation $\rho$. Consider the coloration of $G$ given by $\col \rho$. It is clearly valorable, so we need only show it is va
\end{proof}

\subsection{Direct implication}

In the interest of characterizing the relation of indirect implication in the context of this kind of multigraph, we will consider the following definition:
