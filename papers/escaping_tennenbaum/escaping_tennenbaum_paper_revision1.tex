...
$\mathsf{ZFC}$, though we will not explore this avenue in this paper.

\subsection{Pakhomov's Construction and New Results}

In Fedor Pakhomov's paper ``How to Escape Tennenbaum's Theorem''
\cite{pakhomov}, the author constructs a theory which is definitionally
equivalent to $\ZFniptc$ and admits computable nonstandard models.
We start by reviewing his results.
\begin{definition}
...
definitionally equivalent to $\ZFniptc$. We let $T(\in,S)$ be a
...
signature containing $S^{i}$, $\dots$, and $S^{n}$, there is a
computable copy $M$ of $D\upto(S^{i+1},\dots,S^{n})$.
\end{itemize}
...
apply our \thmref{main_final} to in order to obtain a jump inversion
result, while Theorem 2.5 from \cite{strong_jump_inversion} is relatively
...
our signature includes, for every predicate $P$, its negation $\neg P$
or not.
\end{remark}
In order to introduce our model-theoretic definition, we need to recall
some concepts from computable structure theory. We introduce a new
formula complexity hierarchy.

\begin{definition}
\label{def:comp_inf_lang}We shall work in the Computable Infinitary
Language, as described in \cites[Chapter III]{montalban_cst2}.
...
\[
D\vDash\forall_{\vec{x}}\biggl[\exists_{y}\left[\chi_{Q}(\vec{x},y)\land\tau_{q\varphi}(\vec{x},y)\right]\leftrightarrow\etau_{q\varphi Q}(\vec{x})\biggr].
\]
...
\end{thm*}
We have been able to obtain pre-existing results from the literature
as corollaries of \thmref{main_final}, some examples of which are
found in \secref{prevs_applications}.
\begin{remark}
...
Author's note: An extended version of this article can be found in
\cite{self_escaping_tennenbaum}.
...
will be necessary in the sequel, regarding the new types of formulas
we introduced in \defref{comp_inf_lang}, and regarding the definition
of the QETP, \defref{qetp}.

\begin{remark}
\label{rmk:llorc}The main properties of the $\llorc_{1}$ formulas
are the following:
\begin{itemize}
\item There is a canonical way to associate indices to $\llorc_{1}$ formulas,
so that a computer program may manipulate them, and
\item Given a c.e.\ index for the atomic $\Lang'$-type of a tuple of elements
$\vec{b}$ in some structure, and a $\llorc_{1}$ formula $\varphi(\vec{x})$
...
\item There is a canonical way to turn an index for a $\llorc_{1}$ formula
into an index for a logically equivalent $\llorc_{2}$ formula, and
\item Given a tuple of elements $\vec{b}$ of a structure and a c.e.\ index
...
holds. At that point, return continuously guessing `no', and do
so at least once, and go back to finding a (new) value $j$ for which
$\psi_{j}(\vec{b})$ holds, and so on. This sequence of guesses will
...
%
\begin{remark}
\label{rmk:sigma1c}Compare the properties in \rmkref{llorc} to the
following properties of a related class of computable infinitary formulas.
The $\Sigma_{1}^{\mathrm{c}}$ formulas are those formulas in the
Computable Infinitary Language of the form:
\[
...
\begin{itemize}
\item The $\Sigma_{1}^{\mathrm{c}}$ formulas also admit a canonical way
to associate indices, and
\item Given a tuple $\vec{b}$ of elements in a computable structure $D$
that satisfy the $\Sigma_{1}^{\mathrm{c}}$ formula $\varphi(\vec{x})$,
we can confirm in finite time that the tuple $\vec{b}$ satisfies
the formula $\varphi$.
\end{itemize}
Despite the similarities, the $\llorc$ formulas shall be more useful
in our treatment, because to check the truth of a $\Sigma_{1}^{\mathrm{c}}$
...
%
\begin{remark}
\label{rmk:fokinaetal}Bazhenov, Fokina, Rossegger, Soskova, and Vatev
introduced in \cite{lopez_escobar_cts_domains} two hierarchies of
infinitary formulas, which they named $\Sigma_{\alpha}^{p}$ and $\Pi_{\alpha}^{p}$
for $\alpha<\omega_{1}$, which are defined with positivity of predicates
in mind.\footnote{We would like to thank an anonymous reviewer for bringing this work
...
Implicitly, we use the fact that finite conjunctions of $\llorc_{1}$
formulas are themselves (uniformly equivalent to) $\llorc_{1}$. This
new definition of $\bar{\chi}$ still serves as a witness to the QETP,
if the previous $\chi$ did, but has this additional technical benefit
that will be necessary in our proof.
\end{remark}
...
\item We may match a triplet of formulas $(q,\varphi,\tau)$ to an unmatched
...
of $D$, if that element has a triplet $(q,\varphi,\tau)$ assigned,
...
\item \label{enu:mainstep1}To begin, find the first element of $M$, if
any, that does not have a match in $D$. If there is none, skip to
\ref{enu:mainstep2}. If there is one, call it $m$.
\item If $m$ has an assigned triplet of formulas $(q,\varphi,\tau)$, skip
to \ref{enu:mainstep1.4}.\footnote{Technically, this instruction is unnecessary. The only way that the
condition could be met is in the case of injury (namely, if the triplet
...
\item \label{enu:mainstep1.3}If $m$ does not have an assigned triplet:
...
the triplets $(q,\varphi,\tau)$ enumerated as per \rmkref{tauwork}
until you find such a triplet that satisfies $M\vDash q(\vec{x},y,\vec{z})$
...
are real, injury will occur. Once such a triple is found, we assign
to $m$ the triplet $(q,\varphi,\tau)$ with the decomposition $(\vec{x},y,\vec{z})$
above.
...
$D\vDash\exists_{y}\tau_{q}(\vec{b},y)$, and any witness to this
...
of $\vec{b}$ are all real, and that every matched triplet $(q_{0},\varphi,\tau)$
...
$D\vDash\tau(\vec{b}_{0},d_{0})$ by \enuref{asmtau}, hence $D\vDash\exists_{y}\left[\chi_{Q}(\vec{b}_{0},y)\land\tau_{q\varphi}(\vec{b}_{0},y)\right])$,
...
and $m$ was created by \ref{enu:mainstep1}, restart the execution
of worker $i$. If the match between $d$ and $m$ was created by
the \ref{enu:mainstep2}, initialize worker $i+1$ (and do not restart
worker $i$).
...
past the moment that this formula was assigned to $m$. Then, execute
worker $i$ starting in \ref{enu:mainstep1.3}.
...
retain the assigned triplet $(q,\varphi,\tau)$), and execute worker
...
matches potentially created by worker $i$:
\begin{itemize}
...
$\tau(\vec{b},d)$ at the current approximation, which will nevermore
change. Thus, past this point, this match will never be injured.
\item Now, assuming that we've reached a time where the matching discussed
in the previous bullet point or previous workers will never suffer
...
All of the following results are bidirectional, but due to the focus
...
We have been able to obtain the following three results as a consequence
of \thmref{main_final} without much work. For the sake of space,
we state them without proof. Proofs may be found in \cite{self_escaping_tennenbaum}.
We particularly highlight the first of these, \thmref{JIlinear-1},
whose proof we find interesting, despite being more technically elaborate
than the standard relatively simple finite injury construction.
...
\item $S(x,y)$, meaning that $x$ is the parent of $y$,
\item $R_{0}(x)$, meaning that $x$ is the root of $T$.
\end{itemize}
...
where $N$ is positive and large enough to ensure $\forall_{d}\chi_{q}(\vec{0},Nd)$.
...
signature $\Lang$, and $\vec{b}$ is a tuple in $G$ such that $G\vDash\exists_{\vec{y}}q(\vec{b},\vec{y})$,
...
\end{lemma}
The proof of this lemma is left to the reader. The reader will want
to use a change of variables, similar to the one used in the construction
of $\chi_{q}$, and make use of \ref{rmk:nonzeroxi}.
\end{itemize}
\end{itemize}
...
computable nonstandard models? We show that the answer is yes.
...
plus the axiom $\forall_{x,y}[S^{0}(x,y)\leftrightarrow(x\notin y)]$.
...
-- and hence, by a theorem of Knight \cite{knight_degrees_coded_in_jumps},
...
is definitionally equivalent to ``$\mathsf{PA}$+ all $\Pi_{n}$
