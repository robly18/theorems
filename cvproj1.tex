...
\begin{teorema}
Seja $u$ um minimizante do funcional $I$ sob as condições de fronteira usuais ($u(a) = \alpha$, $u(b) = \beta$), juntamente com a restrição $g(u(x)) = c$, em que $g : \R^d \to \R^k$ e $c \in \R^k$ é um valor regular de $g$. Impõem-se as hipóteses de regularidade: $u$ é $C^2$, $f$ é $C^1$ e $g$ é $C^2$ num aberto que contém o caminho. Então, existe uma função $C^1$, $\lambda : \left]a,b\right[ \to \R^k$, tal que
\[\diffp f u(t,u(t),u'(t)) - \diff*{\left(\diffp f \xi(t,u(t),u'(t))\right)}t = g'(u(t)) \lambda(t).\]
\end{teorema}
