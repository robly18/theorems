...
\begin{theorem}
If $g_{bs}(t) \leq T t^s$, the following inequality is true:
...
where $\Theta$ is the smallest $T$ in these conditions.
\end{theorem}
...
For each $b$ and $s$, we there exists a set $E$ satisfying
...
\begin{theorem}
Fix $s = 1/2$. If $C$ is the optimal constant defined above, we have
...
As mentioned in the introduction, it appears that, the bigger $b$ is (that is, the more pieces we divide our intervals in as $j$ decreases) the bigger the difference between $\HH_s$ and $\BB_{bs}$. This makes sense, since as $b$ increases there is less we can discern because since there is a larger discrepancy between the size of one layer and the next we lose the ability to gauge more precisely the intermediate sizes.

This intuition proves to be partially correct. Indeed, if $b$ and $c$ such that $c$ is a power of $b$, it is easy to check that any $b$-ary interval is also a $c$-ary interval, which implies the inequality $\BB_{bs} \leq \BB_{cs}$. This argument, however, is not generalizable.

It would be nice to have a criterion to, given two numbers $c, b \in \N_{\geq 2}$, tell if it is the case that $\BB_{bs} \leq \BB_{cs}$. One might bodly venture that the $b$-ary measures are, in a sense, monotone as a function of $b$, but this turns out not to be the case, we we show at the end of this article. In other words, the following statement is \textbf{false}:
...
We want to determine the smallest $C$ such that for any interval of length $\ell$ we can find a dyadic covering with Hausdorff sum less than or equal to $C \ell^s$. This can be written as
...
In this case, let $I'$ be the interval given by translating $I$ by $-k b^{j+1}$. Two easy observations are in order.
