...
\begin{definition}\label{def:murs}
...
In this section we show, using corollary \ref{calcmaslov1}, that in the two-dimensional case, [[...]]. But first, we prove the lemmas necessary to ensure that the Maslov index \textit{a la} Robbin and Salamon is well-defined.
...
\item Suppose that $t$ is a nondegenerate crossing of type two. Then, it is possible to verify from the fact that $\diff{}t[t] \det(A(t))$ and $A(t) = I$ we have
\begin{equation}
\begin{cases}
\diff{}t[t] \trace A(t) = 0,\\
\diff[2]{}t[t] \trace A(t) = - 2 \det \dot A(t).
\end{cases}
\end{equation}
...
\centering
\begin{tikzpicture}
\draw[->, thick] (0,-2) -- (0,2) node[right] {$\trace A(t)$};
\draw[thick] (-0.4, 0) node[left] {$2$} -- (12, 0);

\begin{scope}[xshift=1.5cm]
\draw (-1,-1.5) -- (1,1.5);
\draw (0,-2) node {Type one} (0,-2.5) node {$\trace \dot A(t) > 0$};
\end{scope}
...
\draw (-1,1.5) parabola bend (0,0) (1,1.5);
\draw (0,-2) node {Type two} (0,-2.5) node {$\det \dot A(t) < 0$};
\end{scope}
...
This allows us to define the partition to be used for application of corollary \ref{calcmaslov1}.
\begin{algorithm}[Construction of the partition]
\begin{enumerate}[algorithm]
\item Set $a_0 = 0$, $j = 0$, and $k = 0$.
\item If $\det \dot A(0) < 0$, go to \ref{alg1:up}. Otherwise go to \ref{alg1:down}.

\item\label{alg1:up} Set $k \leftarrow k+1$.
\item If $t_k$ is of type two, ignore it and return to \ref{alg1:up}. Otherwise, set $a_j = t_k$ and $j \leftarrow j+1$. Go to \ref{alg1:down}

\item\label{alg1:down} Set $k \leftarrow k+1$.
\item If $t_k$ is of type two, 
\end{enumerate}
\end{algorithm}
\end{proof}