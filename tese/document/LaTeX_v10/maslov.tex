...
By inspecting how the matrix of $\Gamma(A,t)$ changes under change of basis it is possible to check that its characteristic polynomial is independent of basis, and hence that the notions of degeneracy and sign of $\Gamma(A,t)$ are well-defined.
...
The definition we gave above for degeneracy of the 2-tensor $\Gamma(A,t)$ is that the matrix which represents it in an arbitrary basis of $V$ has no zero eigenvalues. It is possible to check that this definition is equivalent to the following:
\begin{equation}
...
Let $A$ be a differentiable path of symplectic matrices and define $\Gamma(A,t)$ and $V$ as above, where $t$ is a crossing. Then, $\Gamma(A,t)$ is symmetric on $V$.
\end{lemma}
