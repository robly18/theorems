...
In this section we show using corollary \ref{calcmaslov1} that, in the two-dimensional case, $\murs(A(t)) = \mu(A(t))$. But first, we prove the lemmas necessary to ensure that the Maslov index \textit{a la} Robbin and Salamon is well-defined.
...
\fill[color=magenta!10] (2,-3) -- (2,3) -- (4,3) -- (4,-3) -- cycle;
\fill[color=blue!10] (2,0) to [bend right] (-2,0) -- (-2,3) -- (2,3) -- cycle;
\fill[color=cyan!10] (2,0) to [bend left] (-2,0) -- (-2,-3) -- (2,-3) -- cycle;
\fill[color=gray!10] (-2,-3) -- (-2,3) -- (-4,3) -- (-4,-3) -- cycle;

\draw[white, very thick] (-2,-3)--(-2,3) (2,-3)--(2,3);
\draw (2,0) to [bend left] (-2,0) to [bend left] (2,0);
...
\draw[thick, ->] (-5,0) -- (5,0) node[below] {$\trace A$};
\end{tikzpicture}
...
\begin{algorithm}[Construction of the partition]\label{alg:p}
...
\begin{algorithm}[Sign of a crossing $t$]\label{alg:scrossing}
