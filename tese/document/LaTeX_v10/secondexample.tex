...
\begin{remark}\label{rmk:assumenondegen2}
All the statements in remark \ref{rmk:assumenondegen1} regarding the regularity of the canonical almost-complex structure wrt $H_1$ hold, with one exception.

While in the previous chapter the symmetries of $J$ were not essential for computation, as the fact that Floer homology agrees with Morse homology was sufficient to compute the boundary maps with no room for doubt, in the present example there \emph{is} some ambiguity arising from this method. Fortunately, this ambiguity is present only in the boundary maps of the filtered Floer complex, and does not extend to the barcodes, which are entirely determined, and hence the barcodes computed below are the barcodes of $\phi$ regardless of whether the pair $(H_1,J)$ is regular or not.
\end{remark}
...
Note: See remark \ref{rmk:assumenondegen2}.
