...
def f(x):
	...
	Note-se que esta afirmação, por si só, não faz perfeito sentido: as tautologias são fórmulas proposicionais, enquanto que estamos no contexto de fórmulas de primeira ordem. É então preciso fazer um ligeiro esforço para clarificar o significado.
	...
	\end{equation}
	visto que se $I$ é um grupo que satisfaz $\alpha$, temos que $I$ é necessariamente o grupo trivial, visto que para qualquer valoração $\rho$ temos $\rho(x) = \mathbf{id}_I$. O grupo trivial satisfaz também $\beta$, pelo que a afirmação \eqref{mtd:contraex1} é verdadeira.
