...
\begin{document}
	...
	Para definir $\rho(\bf x_1)$, averigue-se se $\Gamma, \bf x_1 \vdash \varphi$.
	
	\begin{itemize}
	\item Em caso afirmativo, o nosso $\rho$ não poderá atribuir $\rho(\bf x_1) = \lt$, ou então o `sistema' a resolver ficava impossível! Assim sendo, definimos $\rho(\bf x_1) = \lf$. 
	...
	Se $\rho(\bf x_N) = \lt$, então por construção isso significa que $\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}), \bf x_N \nvdash \varphi$, que, como neste caso $r(\bf x_N)$ é $\bf x_N$, mostra o desejado.
	
	Se $\rho(\bf x_N) = \lf$, então, por construção, temos que
	\begin{equation}\label{eq:comp:prop:1}
	\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}), \bf x_N \vdash \varphi.
	\end{equation}
	
	Faremos os detalhes mais tarde (ver lema \ref{lemma:1}), mas a ideia é a seguinte: supondo, por absurdo, que
	\begin{equation}\label{eq:comp:prop:2}
	\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}), \neg \bf x_N \vdash \varphi,
	\end{equation}
	ter-se-ia, juntando \eqref{eq:comp:prop:1} e \eqref{eq:comp:prop:2},
	\[\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}), \bf x_N \lor \neg \bf x_N \vdash \varphi.\]
	
	Sabendo que a hipótese $\bf x_N \lor \neg \bf x_N$ é uma tautologia, pode ser retirada das hipóteses (ver lema \ref{lemma:2}), pelo que concluímos
	\[\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}) \vdash \varphi,\]
	uma contradição com a hipótese de indução. Conclui-se então que
	\[\Gamma, r(\bf x_1), \dots, r(\bf x_{N-1}), \neg \bf x_N \nvdash \varphi,\]
	como desejado.
	\end{itemize}
	
	Podemos agora mostrar $\rho \nVdash \varphi$. Para o fazer, repare-se que se $\rho \Vdash \varphi$ ter-se-ia (ver lema \ref{lemma:3})
	\[r(\bf x_1), r(\bf x_2), \dots \vdash \varphi,\]
	o que implicaria (visto que não há problema em adicionar hipóteses) que
	\[\Gamma, r(\bf x_1), r(\bf x_2), \dots \vdash \varphi,\]
	o que entra em contradição com \eqref{eq:comp:prop:0}.
	
	Mostre-se, agora, que para todo $\gamma \in \Gamma$ se tem $\rho \Vdash \gamma$. Usando, novamente, o lema \ref{lemma:3}, temos que, assumindo por absurdo $\rho \nVdash \gamma$, ter-se-ia $\rho \Vdash \neg \gamma$ e como tal
	...
	\Gamma, r(\bf x_1), r(\bf x_2), \dots \vdash \gamma
	\end{gather}
	a primeira por \eqref{eq:comp:prop:3}, e a segunda porque $\gamma \in \Gamma$. Usando o princípio da explosão (lema \ref{lemma:4}), concluímos
	\[
	\Gamma, r(\bf x_1), r(\bf x_2), \dots \vdash \varphi
	\]
