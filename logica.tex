...
\begin{document}
	...
	De modo a tentar determinar quais serão os axiomas que faltam, tentaremos determinar um algoritmo geral para demonstrar tautologias. Um aspeto principal a saber é que \emph{algo} tem que fazer a ligação entre valorações e demonstrações. Na nossa demonstração de completude, este papel coube ao lema \ref{lema:3}. No entanto, a sua demonstração requeriu o uso da completude no caso finito, que por sua vez requer o conhecimento que é possível mostrar qualquer tautologia. Temos, então, de tentar descobrir uma forma de demonstrar este lema de forma mais construtiva.
	...
	Extendemos a definição de $r$ como $r(\varphi)$ sendo $\varphi$ ou $\neg \varphi$ consoante o valor de $\overline\rho(\varphi)$. Definimos também $\Gamma_\rho = \{r(\bf x_1), r(\bf x_2), \dots\}$. Pretendemos mostrar que
	...
	Necessitamos, então, de mostrar as seguintes três tautologias, todas usando o operador `não':
	\begin{gather}
	\alpha \imply \neg \neg \alpha\label{ax3}\\
	...
	Usando \eqref{cr1} como axioma, a fórmula \eqref{ax5} pode também ser demonstrada de forma semelhante. Isto sugere então a adição de \eqref{ax3}, \eqref{cr1} e \eqref{cr2} como axiomas. Acontece, no entanto, que todos exceto \eqref{cr2} são redundantes. Justificaremos isto lentamente, começando por justificar o complementar de \eqref{ax3}. Isto é, vamos mostrar que $\neg \neg \alpha \imply \alpha$.
	
	Visto que a conclusão é uma implicação que não tem negações no seu lado direito, não é razoável esperar resolver isto usando \eqref{ax3} ou \eqref{cr1}, pelo que teria que ser através de \eqref{cr2}. Examinando este axioma, conclui-se que seria necessário provar a implicação $\neg \alpha \imply \neg \neg \neg \alpha$, ou, aplicando este axioma mais uma vez (!!), $\neg \neg \neg \neg \alpha \imply \neg \neg \alpha$.\footnote{Lamento sinceramente o exagero de negações. Se algum dia descobrir uma demonstração menos esotérica, a primeira coisa que faço é substituir esta.} Ora, partindo da hipótese $\neg \neg \alpha$ isto é uma aplicação direta do axioma $\neg\neg\alpha \imply (\neg\neg\neg\neg\alpha \imply \neg\neg\alpha)$,\footnote{Se o leitor conhecer alguma prova mais direta, por favor informe-me. A sério.}, pelo que chegamos à conclusão que
	\[\neg \neg \alpha \vdash (\neg \neg \alpha \imply \alpha).\]
	
	Aplicando MP seguido do MTD, chegamos à conclusão desejada:
	\[\neg\neg\alpha \imply \alpha.\]
