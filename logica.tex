...
def f(x):
	...
	\begin{definicao}
	Em tudo o que se segue, considera-se fixa uma afirmação verdadeira $\tau$ na assinatura $\Sigma$.\footnote{É fácil construir afirmações verdadeiras, desde que haja pelo menos uma fórmula. Por exemplo, a partir de uma fórmula qualquer em $\F_\Sigma$ é possível construir uma tautologia. Mas para isto é necessário que $\F_\Sigma \neq \emptyset$. Assim sendo, ao dizer isto, estamos implicitamente a assumir que o conjunto de símbolos de proposição, $P_\Sigma$, é não-vazio. Claro que o caso contrário é inútil, visto que nesse não há fórmulas de todo.}
	...
	É possível adaptar um raciocínio `incremental' como o que foi feito para o caso proposicional (onde íamos adicionando condições à medida que estabelecíamos valores para variáveis) mas no que se segue tomamos uma abordagem diferente.
	\end{obstaculo}
