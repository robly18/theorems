...
def f(x):
	...
	Primeiro que tudo, note-se que, se $\varphi$ é uma fórmula fechada, ou $\psi$ também o é ou então $\fv \psi = \{\bf x\}$. O caso em que $\psi$ é uma fórmula fechada é o mais fácil, porque é fácil justificar que, tanto semanticamente como sintaticamente, $\forall_{\bf x} \psi$ é equivalente a $\psi$. Isto é:
	\[\widehat\Gamma \vdash \forall_{\bf x}\psi \text{ sse } \widehat\Gamma \vdash \psi \text{ sse (H.I.) } I \Vdash \psi \text{ sse } I \Vdash \forall_{\bf x} \psi.\]
	\end{itemize}
	\end{proof}
